...
export function animateServices(container: HTMLElement): void {
    gsap.registerPlugin(ScrollTrigger);
    const pref = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    const title = container.querySelector<HTMLElement>('.titleContainer');
    const lead = container.querySelector<HTMLElement>('.servicesLead');
    const cards = Array.from(container.querySelectorAll<HTMLElement>('.serviceCard'));
