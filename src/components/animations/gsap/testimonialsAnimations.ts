...
export function animateTestimonials(container: HTMLElement): void {
    gsap.registerPlugin(ScrollTrigger);
    const pref = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    const title = container.querySelector<HTMLElement>('.titleContainer');
    const lead = container.querySelector<HTMLElement>('.testimonialsLead');
    const cards = Array.from(container.querySelectorAll<HTMLElement>('.testimonialCard'));
